...
export default function Login() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // Step 2: fetch teams — token is now in memory so this works
      // Sets activeTeamId + activeRole in store + persists to localStorage
      try {
        const { data: td } = await api.get('/teams');
        const teams = td.teams || [];
        if (teams.length > 0) {
          setActive(teams[0].id, teams[0].role);
        }
      } catch {
        // non-fatal
      }
        ...
        <div className="card" style={{ padding:28 }}>
          ...
          <form onSubmit={submit}>
            <div className="field">
              ...
              <input className="input" type="email" placeholder="you@company.com" value={f.email} onChange={e => setF(p=>({...p,email:e.target.value}))} required />
            </div>
            <div className="field">
              <label className="label">Password</label>
              <input className="input" type="password" placeholder="••••••••" value={f.password} onChange={e => setF(p=>({...p,password:e.target.value}))} required />
            </div>
